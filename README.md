...
> [Download here the Grasshopper definition](https://github.com/angelpontes/PFC/PARAMETRIC DESIGN/HEX PARAMETRIC SYSTEM v1 FOR 3D PRINTING IN MILLIMETERS.gh "GH Sheet")
...
Once all the parameters have been modified the file GH produce the 2D and 3D components required for the digital fabrication. While the 2D hexagonal grid could be cut by laser, the 3D components have to be printed. The defined model unit is millimeter.

Weaver Bird plugin for GH have to be installed. Lunchbox plugin for GH should be installed to bake the files with the default variables.
